...
namespace raven::ASM {
    ...
    class Xor : public Instruction {
        ...
        public:
            Xor() = default;
            template<typename T>
            Xor(const Register &dst, const T &src) : _dst(dst), _src(src) {
                update();
            }
            ~Xor() = default;

            Xor &setDst(const Register &dst);

            template<typename T>
            requires std::is_same_v<T, Register> || std::is_same_v<T, Immediate>
            Xor &setSrc(const T &src) {
                _src = src;
                update();
                return *this;
            }

            static std::vector<std::shared_ptr<Instruction>> xor_(const Register &dst, const Register &src);
            static std::vector<std::shared_ptr<Instruction>> xor_(const Register &dst, const int64_t &src);
